...
String dnm=request.getParameter("category"); 
...
p2.add("Department wise report for Department "); //no alignment
p2.add(dnm+" for status: "+stat+"\n");
...
ResultSet rs=st.executeQuery("SELECT items.sub_cat,specification.mech,specification.spec, items.dsrno,items.qty,items.status,"
        ...
        + "ON items.type = specification.type where items.status='"+stat+"' AND items.dept_name='"+dnm+"'");
...
PdfPCell cellTwo = new PdfPCell(new Phrase("Name and Sign of Lab \nIncharge"));
