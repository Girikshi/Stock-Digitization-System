...
ResultSet rs=st.executeQuery("select "
        + "items.type,items.sub_cat,items.purchase_num,status,lab_id,mech,spec,extra,repair,items.purchase_date from items join specification on(items.type=specification.type) where dsrno='"+dsrno+"'" ); 
...
window.location.href = "http://localhost:8080/integrate/indexla.html";</script>
<%
}
else
{
...
window.location.href = "http://localhost:8080/integrate/indexla.html?demo=<%=request.getParameter("demo")%>&dn=<%=request.getParameter("dn")%>&useri=<%=request.getParameter("useri")%>";</script>
<%
