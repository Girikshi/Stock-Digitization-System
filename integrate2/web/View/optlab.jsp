    ...
    function handleClick1( form )
{
    ...
    {
        form.action = "labpdf.jsp?demo=<%=request.getParameter("demo")%>&dn=<%=request.getParameter("dn")%>&useri=<%=request.getParameter("useri")%>";
        form.target="_blank";
    ...
    function handleClick2( form )
{
    ...
    {
        form.action = "hodLabChart.jsp?demo=<%=request.getParameter("demo")%>&dn=<%=request.getParameter("dn")%>&useri=<%=request.getParameter("useri")%>";
        form.target="_blank";
...
%>
  <form name="f1"  method="post" >
    ...
    try
    {
        ...
        pstmt=con.prepareStatement("select distinct lab_id from labs where dept_name="+dnm+" and lab_name!='None'"); //sql select query
